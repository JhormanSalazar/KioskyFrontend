        ...
        <div ref="sceneRef" class="grid w-full h-full" :style="sceneStyle">
            <template v-for="(_, r) in cells" :key="`row-${r}`">
                <div v-for="(__, c) in cells" :key="`${r}-${c}`"
                    class="relative w-full h-full aspect-square transform-3d cube" :data-row="r" :data-col="c">
                    <span class="absolute -inset-9 pointer-events-none" />

                    <div class="absolute inset-0 flex justify-center items-center cube-face" :style="{
